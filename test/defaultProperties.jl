...
@testset "Wavelet properties" begin
    ...
    cwts = (cDb2, dog1, dog2, paul1, morl, morse)
    βs = (1, 4)
    ns = (128, 1382, 2039)
    averagingLengths = (0, 0.5, 1, 2, 4)
    @testset "length $n, with type $wave, bc $bc, β=$β, ave=$(ave)" for n in ns,
        wave in cwts,
        bc in bcs,
        β in βs,
        ...
        supported = spaceWaves[[1:floor(Int, n / 2)

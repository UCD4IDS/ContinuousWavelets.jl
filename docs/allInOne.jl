...
scatter(1:length(locs),
    ...
    color = :black)
scatter!(length(locs):length(locs),
    locs[end:end],
    markersize = 10,
    markershape = :x,
    color = :black,
...
curve = b .*
        (range(firstW, stop = (locs[end] / b)^dRate, length = length(t))) .^ (1 / dRate)
plot!(t,
    curve,
    color = :blue,
    line = :dash,
    ...
    yticks = (range(floor(Int, minimum(locs)), ceil(Int, maximum(locs)), step = 2),
...
exs = cat(testfunction(n, "Doppler"),
...
p2 = plot(heatmap(identity.(res[:, :, 1])',
        ...
        c = :viridis,
        colorbar = false),
    heatmap(identity.(res[:, :, 2])',
        xticks = false,
        yticks = [],
        c = :viridis,
        colorbar = false),
    heatmap(identity.(res[:, :, 3])',
        xticks = false,
        yticks = [],
        c = :viridis,
        colorbar = false),
    heatmap(identity.(res[:, :, 4])',
        xticks = false,
        yticks = [],
        c = :viridis,
        colorbar = false),
...
plot([dualframe dualPenrose dualNaive],
